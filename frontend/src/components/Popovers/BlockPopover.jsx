...
export default function BlockPopover() {
  ...
  const { CloseBlockPopover } = usePopoversStore();

  const { blockUser, isBlocking } = useFriendsStore();
    ...
    >
      ...
      >
        ...
        <div className="gap-5  p-2 flex flex-wrap justify-center items-center flex-row">
          ...
          >
            {isBlocking ? <SendLoader color={myMessageTheme} /> : "Block"}
          </button>
          <button
            onClick={CloseBlockPopover}
