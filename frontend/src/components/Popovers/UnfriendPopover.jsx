...
export default function UnfriendPopover() {
  const { authUser } = useAuthStore();
  const { SelectedUser, SetSelectedUser } = useChatStore();
  const { CloseUnfriendPopover } = usePopoversStore();
  const { unfriendUser, isUnfriending } = useFriendsStore();
    ...
    >
      ...
      >
        ...
        <div className="gap-5  p-2 flex flex-wrap justify-center items-center flex-row">
          ...
          >
            {isUnfriending ? <SendLoader color={myMessageTheme} /> : "Unfriend"}
          </button>
          <button
            onClick={CloseUnfriendPopover}
